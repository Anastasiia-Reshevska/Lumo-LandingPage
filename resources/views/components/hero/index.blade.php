<section class="hero">
  <div class="hero__wrapper-content">
    <div class="hero__content">
      <p class="hero__tagline">EMBRACE THE CHILL</h3>
      <h1 class="hero__title">LUMO’S <span>WINTER FEST</span> COLLECTION <span>2024-2025</span></h1>
      <p class="hero__subtitle">Stay warm, stylish, and ready for winter.</p>
      <div class="hero__button">
        <button class="btn-primary btn-gray hero__btn-main">PREP FOR THE WINTERFEST
          <div class="arrow arrow--medium" aria-hidden="true">
            <div class="arrow__direction arrow__direction--white"></div>
          </div>
        </button>
      </div>
      <p class="hero__text">CHECK LAST YEAR’S EDITION</p>
    </div>
  </div>
  <div class="hero__image-wrapper">
    <div class="hero__image" aria-hidden="true"></div>
  </div>
</section>
